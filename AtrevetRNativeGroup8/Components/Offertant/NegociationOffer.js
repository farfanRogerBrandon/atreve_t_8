...
const NegociationOffer = () => {
  ...
  const [iOffer, setIOffer] = useState(false);
  ...
  const susc = async () => {
    try {
      ...
      const unsub = onSnapshot(doc(db, "offer", offer.id), async (doc) => {
        if (doc.data().statusNegociation == 0 ) {
          try {
  ...
  const makeOffer = async () => {
    ...
    let res = await makeCostOffer(priceo, msg, offer.id, 1);
    if (res) {
  ...
  const rejectOffer= async()=>{
    try {
      let res = await RejectOffer(offer.id, 1);
      if(res){
    ...
    <View style={ListCarStyles.container}>
      ...
      <ScrollView>
        <View style={{ backgroundColor: "white", padding: 10, borderRadius: 10, elevation: 7 }}>
          ...
          <Text style={{ fontWeight: "bold", fontSize: RFValue(16) }} > Fecha: <Text style={{ fontWeight: "normal" }}>{offer.data.beginDate}</Text> </Text>
          ...
          <Text style={{ fontWeight: "bold", fontSize: RFValue(19), color: "darkgreen" }}>PRECIOS OFERTADOS:</Text>
          {
            priceOfs.length > 0 ?
              ...
              priceOfs.map((item) => {
                ...
                if(item.im){
                 return(
                  <View style={{backgroundColor:"#986842", padding:10, marginLeft:"auto",  borderRadius:6, margin:5}}>
                  ...
                  return(
                    <View style={{backgroundColor:"#cd9564", padding:10,borderRadius:6, marginRight:"auto", margin:5}}>
...
export default NegociationOffer;
